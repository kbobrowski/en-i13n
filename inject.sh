...
while true
do
  fridarunning=`ps | grep frida-server | wc -l`
  scriptrunning=`ps | grep frida-inject | wc -l`
  if [ $fridarunning -ne 1 ]
  then
    if [ $scriptrunning -eq 1 ]
    then
      echo "[$scriptname] killing inject"
      ps | grep frida-inject | xargs sh -c 'kill -9 $1'
    fi
  ...
  then
    ...
    $fridainject -n com.google.android.gms.persistent -s $allowscript --parameters '{"packageName":"de.rki.coronawarnapp.dev","signatureSha":"854528796DB85A3155FAAF92043CD3C42163CB9FA3C6709324A7F39DF4158462","forcedk":true,"unlimiteddk":true}' &
  fi
  sleep 10
done
